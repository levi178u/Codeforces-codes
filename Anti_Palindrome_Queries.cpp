...
#define gv(v, n)  vector<lli> v(n+1); for (int i = 1; i <= n; i++) { int x; cin>>x; v.push_back(x); }
...
while(p--){
    ...
    int c=0;
    lli p1=x[r]-x[l-1] ,p2=y[r]-y[l-1],p3=z[r]-z[l-1] ;
    id=id/2;
    if(p1>=id){
        c++;
    }
    ...
    if(p1<=id and (p2)<=id and (p3)<=id and c>0){
        yes
    }else{
        no
    }
