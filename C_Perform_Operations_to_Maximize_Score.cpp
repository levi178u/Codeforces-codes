...
typedef vector<pair<lli,lli>> vpi;
...
// int bs(vi arr int low, int high, int x){
//     while(low<=high){
...
int mew(vi xx,int op,int np){
    lli s=0,e=1e10,c=0,ans=0;
     int gg=(np-1)/2+1;
    while(s<=e){
        int mid=s+(e-s)/2;
        for(auto &i: xx){
...
gv(a,n);
gv(b,n);
lli ans=a[n-1],med,fans=0,op=0,maxi=0;
vi x,y,z;
fr(i,n){
    if(b[i]){
        x.pb(a[i]);
...
if(min(x.sz,k)){
   ...
   cout<<maxi<<endl;
    ...
    maxi=max(maxi,y.back()+mew(y,k,n));
    y.pob();
}
