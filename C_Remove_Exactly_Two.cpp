...
void ichigo() {
    ll n, m, x, y, p = 0;
    cin >> n;
    vector<vector<ll>> v(n + 1); //for adjacency list
    vector<ll> adj(n + 1, 0); // for the degree 
    ...
    x = *max_element(adj.begin(), adj.end());
    vector<ll> nodes;
    ...
    //edge case where the degree exceeds
    if (nodes.size() > 2) {
        cout <<- 1;
        return;
    }
    ...
    y = *max_element(adj.begin(), adj.end());
