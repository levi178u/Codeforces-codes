...
int thor(int n,int z,int k,vll ao,vi po){
 int ans=0,sum=0;
 vector<int>done(n,0);
 while( done[z]==0 and k>0){
    done[z]=1;
    ans=max(ans,k*ao[z]+sum);
    sum +=ao[z]; z=po[z]-1;
k--;
 }
 return ans;
...
//___________________________________________COOK HERE_______________________________________________________
    void ichigo(){ 
        ...
        //        psa=y-1; pbo=p[x-1];
        //     }
        //     ans1=max(ans1,bo+h*a[pbo-1]);
        //     ans2=max(ans2,sa+h*a[psa-1]);
        //  h--;
        // }
        ...
        //  ans1=(lli)thor(n,x,k,a,p);
        //  ans2=(lli)thor(n,y,k,a,p);

        fr(i,min(k,n)){
            ans1=max(ans1,(k-i)*a[x-1]+sum);
            sum +=a[x-1]; x=p[x-1];
        } sum=0;
        fr(i,min(n,k)){
            ans2=max(ans2,(k-i)*a[y-1]+sum);
            sum +=a[y-1]; y=p[y-1];
        }
